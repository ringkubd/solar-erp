...
const EMPTY_PAYMENT = { amount: '', discount_amount: '0', payment_method: 'bank_transfer', payment_date: new Date().toISOString().slice(0,10), transaction_ref: '', reference_note: '' };
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 border rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <form onSubmit={recordPayment} className="px-6 py-4 space-y-4">
              ...
              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-1">Discount (BDT)</label>
                <input type="number" step="0.01" min="0"
                  value={payForm.discount_amount} onChange={e => setPayForm({...payForm, discount_amount: e.target.value})}
          ...
          <tbody className="divide-y">
            {(invoice.items || []).map((item: any) => (
              <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                ...
                <td className="px-4 py-3 text-right font-mono text-emerald-600">{item.discount_pct}%</td>
            ...
            <tbody className="divide-y">
              {(invoice.payments || []).map((pay: any) => (
                <tr key={pay.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                  ...
                  <td className="px-4 py-3 text-right font-mono text-xs text-emerald-600">{Number(pay.discount_amount) > 0 ? `৳${Number(pay.discount_amount).toLocaleString()}` : '—'}</td>
