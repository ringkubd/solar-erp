...
export default function MailCompose({ onClose, onSent }: any) {
  ...
  const generateAiDraft = async () => {
    ...
    try {
      ...
      setForm({ ...form, body: res.data.draft });
      setShowAi(false);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-end p-6 pointer-events-none">
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl border pointer-events-auto animate-in slide-in-from-bottom-4 duration-300 flex flex-col overflow-hidden max-h-[80vh]">
        ...
        <form onSubmit={handleSend} className="flex-1 flex flex-col overflow-hidden bg-slate-50/50">
          ...
          <div className="flex-1 p-6 relative">
             {showAi ? (
               <div className="absolute inset-0 z-10 p-6 bg-blue-50/90 backdrop-blur-sm animate-in fade-in duration-200">
